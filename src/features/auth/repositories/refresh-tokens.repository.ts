...
import { RefreshTokensQueries } from "../queries/refresh-tokens.queries.js";

export class RefreshTokensRepository
  extends BaseRepository<RefreshTokenEntity, RefreshToken>
  implements IRefreshTokensRepository
{
  constructor(db: Database, mapper: RefreshTokenMapper) {
    super(db, mapper);
  }
  ...
  async findByUserId(userId: string): Promise<RefreshToken | null> {
    return this.findOne(RefreshTokensQueries.FIND_REFRESH_TOKEN, [userId]);
  }

  async update(userId: string, tokenHash: string, expiresAt: Date): Promise<RefreshToken> {
    return this.saveOne(
