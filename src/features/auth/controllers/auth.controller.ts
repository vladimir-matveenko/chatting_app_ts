...
export class AuthController extends BaseController {
  ...
  async register(
    ...
    const dto = this.validators.register.validate(req);

    const result = await this.authService.register(dto);

    res.status(201).json(this.mappers.response.map(result));
  }
  ...
  async login(
    ...
    const dto = this.validators.login.validate(req);

    const result = await this.authService.login(dto);

    res.json(this.mappers.response.map(result));
  }

  async refresh(req: Request, res: Response): Promise<void> {
    const dto = this.validators.refresh.validate(req);
