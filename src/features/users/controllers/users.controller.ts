...
export class UsersController extends BaseController {
  ...
  async create(req: Request, res: Response<UserResponseDto>): Promise<void> {
    const dto = this.validators.create.validate(req);

    const user = await this.usersService.createUser(dto);

    this.created(res, this.mappers.response.map(user));
  }

  async me(req: Request, res: Response<UserResponseDto>): Promise<void> {
    if (!req.user) {
    ...
    res.json(this.mappers.response.map(user));
  }

  async getById(req: Request, res: Response<UserResponseDto>): Promise<void> {
    const dto = this.validators.getById.validate(req);

    const user = await this.usersService.requireById(dto.id);

    this.ok(res, this.mappers.response.map(user));
  }

  async getByEmail(req: Request, res: Response<UserResponseDto>): Promise<void> {
    const dto = this.validators.getByEmail.validate(req);
  ...
  async updateMe(
    ...
    const dto = this.validators.update.validate(req);
  ...
  async updatePassword(
    ...
    const dto = this.validators.updatePassword.validate(req);
